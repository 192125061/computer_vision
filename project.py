...
for img in glob.glob(dir+"/Images/*.jpeg"):
    ...
    if number_plate is not None:
        ...
        print("No number plate detected for this image.")
...
workbook = openpyxl.load_workbook("license_plates.xlsx")
worksheet = workbook.active

entry_times = [datetime.now(), datetime.now(), datetime.now(), datetime.now()]

for index, (value, entry_time) in enumerate(zip(array, entry_times), start=1):
    worksheet.cell(row=index, column=1, value=value)
    worksheet.cell(row=index, column=2, value=entry_time.strftime("%Y-%m-%d %H:%M:%S"))

workbook.save("license_plates.xlsx")
...
for img in glob.glob(dir+"/search/*.jpeg"):
    ...
    if number_plate is not None:
        ...
        print("No number plate detected for the search image.")